...
const ChatSection = () => {
  ...
  const handleGroupSearch = (query) => {
    ...
    if (query != user?.username) {
      try {
        ...
        fetch(
          `http://localhost:8080/api/v1/users/user?search=${groupCreateSearch}`,
          requestOptions
        )
          .then((response) => response.json())
          .then((result) => {
  ...
  const handleCreateGroup = () => {
    ...
    try {
      ...
      fetch("http://localhost:8080/api/v1/chat/group/create", requestOptions)
        .then((response) => response.json())
        .then((result) => {
          console.log(result);
  ...
  return (
    <>
      {/* <DrawerHeader>
        <DrawerTitle>Sharing is caring.</DrawerTitle>
        <DrawerDescription>One Gossip a day keep stress away</DrawerDescription>
      </DrawerHeader> */}
      <div>
        <Tabs
